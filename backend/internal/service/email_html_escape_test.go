...
func TestBuildVerifyCodeEmailBody_EscapesSiteName(t *testing.T) {
	...
	t.Run("escapes_script_injection", func(t *testing.T) {
		body := svc.buildVerifyCodeEmailBody(context.Background(), "123456", `</h1><script>alert(1)</script><h1>`)
	...
	t.Run("escapes_html_entities", func(t *testing.T) {
		body := svc.buildVerifyCodeEmailBody(context.Background(), "123456", `A&B<C>"D`)

		assert.Contains(t, body, "A&amp;B&lt;C&gt;&#34;D")
	})

	t.Run("normal_site_name_unchanged", func(t *testing.T) {
		body := svc.buildVerifyCodeEmailBody(context.Background(), "654321", "My Site")

		assert.Contains(t, body, "<p class=\"site\">My Site</p>")
...
func TestBuildPasswordResetEmailBody_EscapesSiteName(t *testing.T) {
	...
	t.Run("escapes_html_tags_in_site_name", func(t *testing.T) {
		body := svc.buildPasswordResetEmailBody(context.Background(), "https://example.com/reset?token=abc", `</h1><img src=x onerror=alert(1)>`)
	...
	t.Run("escapes_html_entities", func(t *testing.T) {
		body := svc.buildPasswordResetEmailBody(context.Background(), "https://example.com/reset", `A&B<C>`)
	...
	t.Run("normal_site_name_and_url_unchanged", func(t *testing.T) {
		...
		body := svc.buildPasswordResetEmailBody(context.Background(), resetURL, "Sub2API")

		assert.Contains(t, body, "<p class=\"site\">Sub2API</p>")
	...
	t.Run("escapes_ampersand_in_reset_url", func(t *testing.T) {
		...
		body := svc.buildPasswordResetEmailBody(context.Background(), resetURL, "Site")
