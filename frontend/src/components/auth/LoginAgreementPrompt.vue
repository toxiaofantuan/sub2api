<template>
  ...
  >
    <div class="flex items-start gap-2">
      <input
        ...
        type="checkbox"
        :checked="accepted"
        class="mt-[2px] h-4 w-4 flex-shrink-0 rounded border-gray-300 text-sky-700 focus:ring-sky-300 dark:border-dark-600 dark:bg-dark-900 dark:focus:ring-sky-400/30"
        ...
        <p class="text-[13px] leading-5 text-gray-600 dark:text-dark-300">
          ...
          <template v-for="(doc, index) in documents" :key="doc.id || doc.title">
            <RouterLink
              ...
              class="font-medium text-sky-700 underline-offset-4 transition hover:text-blue-600 hover:underline dark:text-sky-200 dark:hover:text-blue-100"
            >
              {{ doc.title }}
            </RouterLink>
  ...
  <div
    ...
    class="rounded-lg border border-sky-100 bg-sky-50/80 p-3 text-sm text-sky-900 dark:border-sky-400/20 dark:bg-sky-400/10 dark:text-sky-100"
  >
    <div class="flex items-start gap-3">
      <Icon name="shield" size="sm" class="mt-0.5 flex-shrink-0 text-sky-600 dark:text-sky-200" />
      <div class="min-w-0 flex-1">
        <p class="font-medium">{{ t('legal.loginAgreementPrompt.noticeTitle') }}</p>
        <p class="mt-1 text-sky-700 dark:text-sky-100/80">
          {{ t('legal.loginAgreementPrompt.noticeDescription') }}
        </p>
      </div>
      <button
        type="button"
        class="flex-shrink-0 rounded-md bg-sky-500 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-sky-600"
        @click="emit('open')"
      >
        ...
        <div class="w-full max-w-[600px] overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/10 dark:bg-dark-900 dark:ring-white/10">
          <div class="border-b border-gray-100 bg-white px-6 py-6 dark:border-dark-800 dark:bg-dark-900">
            <div class="flex items-start gap-4">
              <span class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-sky-50 text-sky-700 ring-1 ring-sky-100 dark:bg-sky-400/10 dark:text-sky-200 dark:ring-sky-400/20">
          ...
          <div class="max-h-[58vh] overflow-y-auto px-6 py-5">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <RouterLink
                ...
                target="_blank"
                rel="noopener noreferrer"
                class="group flex min-h-[72px] w-full items-center gap-3 rounded-xl border border-gray-200 bg-gray-50/70 px-4 py-3 text-left transition hover:-translate-y-0.5 hover:border-sky-200 hover:bg-white hover:shadow-sm dark:border-dark-700 dark:bg-dark-800/70 dark:hover:border-sky-400/30 dark:hover:bg-dark-800"
              >
                <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-white text-gray-700 ring-1 ring-gray-200 transition group-hover:bg-sky-50 group-hover:text-sky-700 group-hover:ring-sky-100 dark:bg-dark-900 dark:text-dark-200 dark:ring-dark-700 dark:group-hover:bg-sky-400/10 dark:group-hover:text-sky-200 dark:group-hover:ring-sky-400/20">
                ...
                <span class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-gray-400 transition group-hover:bg-sky-50 group-hover:text-sky-600 dark:group-hover:bg-sky-400/10 dark:group-hover:text-sky-200">
          ...
          <div class="border-t border-gray-100 bg-gray-50/80 px-6 py-4 dark:border-dark-800 dark:bg-dark-950/60">
            <div class="grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                class="rounded-xl bg-sky-600 px-4 py-3 text-sm font-semibold text-white shadow-sm shadow-sky-500/20 transition hover:bg-blue-600"
                @click="emit('accept')"
              >
